...
//                    },
        ...
        composable(
            DestinasiDetailAktivitas.routeWithArgs,
            arguments = listOf(
                navArgument(DestinasiDetailAktivitas.ID) {
                    type = NavType.StringType
                }
            )
        ) {
            val id = it.arguments?.getString(DestinasiDetailAktivitas.ID)
            id?.let { nim ->
...
//                    },
        ...
        composable(
            DestinasiDetailPanen.routeWithArgs,
            arguments = listOf(
                navArgument(DestinasiDetailPanen.ID) {
                    type = NavType.StringType
                }
            )
        ) {
            val id = it.arguments?.getString(DestinasiDetailPanen.ID)
            id?.let { nim ->
                DetailScreenPanen(
                    onBackClick = {
                        navController.popBackStack()
...
//                    },
        ...
        composable(
            route = DestinasiUpdatePanen.routesWithArg,
            arguments = listOf(
                navArgument(DestinasiUpdatePanen.ID) { type = NavType.StringType }
            )
        ) { backStackEntry ->
            val id = backStackEntry.arguments?.getString(DestinasiUpdatePanen.ID)
            id?.let {
                UpdateScreenPanen(
                    onBack = { navController.popBackStack() },
                    onNavigate = {
                        navController.navigate(DestinasiHomePanen.route) {
                            popUpTo(DestinasiHomePanen.route) { inclusive = true }
                        }
                    }
                )
            }
